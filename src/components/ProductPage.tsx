...
//       // Fallback to getAllProducts with filter
...
//   // Fallback function if collection endpoint doesn't work
//   const fetchAllProductsWithFilter = async () => {
//     try {
//       const response = await fetch(`${API_BASE_URL}/products?collection=${encodeURIComponent(collectionName)}`);
//       if (response.ok) {
//         const data = await response.json();
//         if (data.success) {
//           setProducts(data.data || []);
...
//             <span className="ml-1 text-[#c9a060]">({selectedColors.length + selectedSizes.length})</span>
//           )}
//         </button>
...
//           {/* Products Grid */}
//           <div className={`flex-1 ${showFilters ? 'lg:w-[calc(100%-18rem)]' : 'w-full'}`}>
//             {loading ? (
//               <div className="flex flex-col items-center justify-center py-20">
//                 <Loader className="w-12 h-12 animate-spin text-[#c9a060] mb-4" />
//                 <p className="text-[#2c1810] font-['Cormorant_Garamond',serif]">Loading products...</p>
//               </div>
//             ) : error ? (
...
//                 <button
//                   onClick={fetchProductsByCollection}
...
//                       {/* Stock Status */}
//                       {product.stock === 0 && (
//                         <div className="absolute top-3 right-3 bg-gray-800 text-white px-3 py-1 rounded-full text-xs">
//                           Out of Stock
//                         </div>
//                       )}
...
//                         <span className="text-[10px] tracking-[2px] text-[#2c1810]/60 font-['Cormorant_Garamond',serif] uppercase">
//                           {product.category}
//                         </span>
//                         <span className="text-[10px] text-[#2c1810]/40">•</span>
//                         <span className="text-[10px] tracking-[2px] text-[#2c1810]/60 font-['Cormorant_Garamond',serif] uppercase">
//                           {product.subCategory}
//                         </span>
//                       </div>
                      
//                       <h3 className="text-[18px] tracking-[1px] mb-3 font-['Cinzel_Decorative',serif] text-[#2c1810] group-hover:text-[#c9a060] transition-colors line-clamp-2">
//                         {product.name}
//                       </h3>
...
// interface ProductPageProps {
//   collectionName: string;
//   showAllProducts?: boolean; // नया prop
//   onBack: () => void;
//   cartCount: number;
//   onCartClick: () => void;
//   onAddToCart: (item: CartItem) => void;
...
// export function ProductPage({ 
//   collectionName, 
//   showAllProducts = false, // default false
//   onBack, 
//   cartCount, 
//   onCartClick, 
//   onAddToCart, 
...
// }: ProductPageProps) {
//   const [products, setProducts] = useState<Product[]>([]);
//   const [allProductsData, setAllProductsData] = useState<Product[]>([]); // सभी products store करने के लिए
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState<string | null>(null);
//   const [showFilters, setShowFilters] = useState(false);
//   const [showSortMenu, setShowSortMenu] = useState(false);
...
//       setError(error instanceof Error ? error.message : 'Failed to load products');
//       // Fallback: सभी products में से filter करें
...
//       });
//     } else {
//       // सिर्फ specific collection के products fetch करें
